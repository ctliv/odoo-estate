...
class PropertyOffer(models.Model):
    ...
    _sql_constraints = [
        ('price', 'CHECK (price > 0)', 'Price must be above zero')
    ]
    ...
    def _check_property_state(record):
        # print(f"Self is {record}")
        # print(f"Property is {record.property_id}")
        # print(f"Name is {record.property_id.name}")
        # print(f"State is {record.property_id.state}")
    ...
    def _check_price(self):
        ...
        for record in self:
            if tools.float_compare(record.price, record.property_id.expected_price * 0.9, precision_digits=1) < 0:
                raise ValidationError("The price cannot be less than 90% of expected_price")
